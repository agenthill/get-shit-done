...
### D5 — Aggressive *within-phase* DAG now; cross-phase pipeline deferred — **RESOLVED (option 4)**
...
**RESOLVED — autonomous cross-phase + rollback protocol shipped (option 4).** The rollback protocol the deferral required is now built (see §"Autonomous cross-phase + rollback protocol (option 4)"). The autonomous loop runs phases with a per-phase checkpoint (`gsd/phase-N-base` annotated tag + byte-snapshot of STATE/ROADMAP/REQUIREMENTS) and an append-only promotion manifest (`.planning/.phase-manifest.json`). A GENUINE failure triggers a clean **Tier-1** rollback of the never-promoted phase and an informed retry up to `git.sdk_max_phase_attempts` (default 5), then HALTs; a transient/quota failure resumes via WAITING.json without consuming an attempt. When a failure is **attributable** to an already-promoted predecessor in the failing phase's `depends_on` closure, a **Tier-2** history-preserving `git revert` unwinds that predecessor (quarantine-move of summaries, ROADMAP/REQUIREMENTS uncomplete, STATE restore from the checkpoint snapshot) and then HALTs — reverting promoted work unattended is itself halt-worthy. The autonomous Tier-2 cascade is **capped at a single attributable predecessor**: a multi-predecessor unwind is `cannot-classify` (human review). A per-step crash-resume journal in `.planning/ROLLBACK.json` lets a crash mid-Tier-2 replay only the remaining steps idempotently on restart (no double-revert). The conservative posture throughout: any inability to decide HALTs with a clean tree rather than guessing.
...
- **`plan-phase`** — `pipeline(research → pattern-map → plan → check)`; fan out per-plan planners (outline → `parallel()` per-plan); revise only changed plans. *CHECK-stage fan-out landed: `dynamic-workflows/gsd-plan-check-fanout.js` runs one read-only branch per PLAN.md (the per-plan gsd-plan-checker dimensions — task completeness, scope, must_haves derivation, key links, context/CLAUDE.md/tier/pattern compliance, scope-reduction, per-task Nyquist presence) plus one cross-plan branch over the full set (the whole-phase dimensions — requirement + decision coverage, dependency graph / cycles, cross-plan data contracts, Wave-0 cross-reference + sampling continuity, research-open-questions resolution, shared-pattern presence). Each branch returns BLOCKER/WARNING findings via schema and writes nothing — collision-free under the no-op isolation because gsd-plan-checker is static analysis (Read/Bash/Glob/Grep, no Write, no test process); the orchestrator aggregates the verdict, drives the revision loop, and owns all writes (STATE.md / ROADMAP.md / the plan commit) and interactive gates. Plan AUTHORING stays serial on the Agent tool (the gsd-planner revision spawn writes PLAN.md files). Non-Claude runtimes fall through to the verbatim single whole-set gsd-plan-checker agent via the runtime gate. The win is collapsing the serial whole-set check from sum → max(slowest-plan-branch, cross-plan).*
- **`map-codebase` / `code-review`** — dynamic mapper set; review-dimension → verify-finding → fix pipeline. *Review-dimension fan-out landed (PR #5): `dynamic-workflows/gsd-code-review-fanout.js` runs 3 read-only dimension reviewers (bugs/security/quality) over the full file set, composed deterministically into a byte-compatible `REVIEW.md` by `compose-review.cjs`; verify-finding deferred; `--fix` stays on the Agent tool.* *Map-codebase focus-area fan-out landed: `dynamic-workflows/gsd-map-codebase-fanout.js` runs the 4 read-only focus-area mappers (tech/arch/quality/concerns) concurrently. Unlike the code-review/verify fan-outs, the mappers WRITE their own docs directly — a deliberate, scoped exception to D2 justified by the **disjoint-filename invariant** (the 4 focus areas map to 7 non-overlapping `.planning/codebase/` filenames, so even under the no-op isolation there is zero file collision and no test process runs → collision-free). Each branch returns `{focus, status, files:[{file, lines}]}` (NOT doc bodies), preserving map-codebase's context-saving design; the orchestrator owns the commit + all interaction (`check_existing`/`scan_for_secrets` gates stay interactive). Non-Claude runtimes fall through to the verbatim `sequential_mapping` path via the runtime gate.*
- **`autonomous` cross-phase** — only after the rollback protocol (D5). *Landed (option 4): `GSD.run` drives the autonomous rollback + informed-retry loop — per-phase checkpoint (`gsd/phase-N-base` tag + STATE/ROADMAP/REQUIREMENTS byte-snapshot under `.planning/.checkpoints/`), append-only promotion manifest (`.planning/.phase-manifest.json` with per-phase `commits` + `depends_on`), Tier-1 clean rollback of a never-promoted failure with informed retry to `git.sdk_max_phase_attempts` (default 5) then HALT, transient/quota resume via WAITING.json, and a Tier-2 history-preserving `git revert` of an attributable promoted predecessor (then HALT). The autonomous Tier-2 cascade is capped at ONE attributable predecessor (≥2 → `cannot-classify`, human review). A per-step crash-resume journal in `.planning/ROLLBACK.json` replays only the remaining Tier-2 steps idempotently after a crash. Tristate classifier posture: any inability to decide HALTs clean rather than guessing. Falsifiers: the per-plan Tier-1/Tier-2 engine tests, the loop-wiring tests, the single-predecessor-cap test, and the crash-resume idempotency test (measure-of-success TEST 2).*
