...
| `project_code` | string | any short string | (none) | Prefix for phase directory names (e.g., `"ABC"` produces `ABC-01-setup/`). New phases match the convention already on disk: if existing phase dirs are bare `NN-slug`, new ones stay bare even when `project_code` is set; the prefix applies when existing dirs are already prefixed or when there are no phase dirs yet. Added in v1.31 |
...
| `git.sdk_max_phase_attempts` | number or null | `5` | **ADR 0013 option 4 (autonomous cross-phase + rollback).** Cap on total GENUINE attempts per phase in the autonomous `GSD.run` loop before halting for a human. After each genuine failure the phase is rolled back (Tier-1) and re-run with the accumulated prior-failure context; the cap bounds those retries. Transient/quota resumes (via WAITING.json) do NOT consume an attempt. `null`/absent → 5 |
